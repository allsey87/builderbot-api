if robot.logger then
   robot.logger.register_module("nodes.place_behavior")
end

return function(data, rules)
   return 
   { type = "selector*", children = {
      -- Is my hand empty, if yes, return true for the whole node and move on
      --                   if not, go search-approach-place
      function()
      ...
      { type = "sequence*", children = {
         ...
         robot.nodes.create_timer_node(
            ...
            function() robot.api.move.with_velocity(-robot.api.parameters.default_speed, 
                                                    -robot.api.parameters.default_speed) end
         ),
         function() robot.logger("INFO", "place_behavior finish") return false, true end
      }}
   }}
end