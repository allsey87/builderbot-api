...
function check_position_in_safe_zone(position, safe_zone)
...
return function(blocks, rules, rule_type)
   ...
   -- match rules
   local possible_targets = generate_possible_targets(structures_in_index_frame, rules.list, rule_type)

   local target = select_target(blocks, possible_targets, rules.selection_method)
